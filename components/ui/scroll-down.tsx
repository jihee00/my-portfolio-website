export default function ScrollDown() {
  return (
    <div className="w-[4rem] h-[4rem] rounded-full border border-primary-foreground grid place-items-center">
      <div className="grid place-items-center w-[1.5rem] h-[2rem]">
        <svg
          width="100%"
          height="100%"
          viewBox="0 0 26 27"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          style={{
            transform: "translate3d(1.13422e-12px, -2.19519e-13px, 0px)",
          }}
        >
          <path
            d="M23.2338 12.28L14.7538 20.8V0.239998H11.3538V20.76L2.87375 12.28L0.59375 14.56L13.0738 27L25.5138 14.56L23.2338 12.28Z"
            fill="white"
          />
        </svg>
      </div>
    </div>
  );
}
